...
pub(crate) async fn get(
	...
	inner_filter = inner_filter.replacen("AND", "WHERE", 1);
	...
	for record_query in sqlx::query_as::<_, RecordQuery>(&format!(
		r#"
			...
			FROM (
			  ...
			  {inner_query}
			  {inner_filter}
			  GROUP BY r_inner.mode_id, r_inner.course_id, r_inner.teleports
			) AS pb
			JOIN records AS r
			...
			ORDER BY r.course_id, c.stage ASC, r.time, r.created_on DESC
			LIMIT {limit}
		"#,
	))
	.fetch_all(&pool)
