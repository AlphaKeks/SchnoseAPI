use {
	crate::{
		models::{Response, ResponseBody},
		GlobalState,
	},
	axum::{
		extract::{Query, State},
		Json,
	},
	chrono::Utc,
	color_eyre::eyre::eyre,
	database::{
		crd::read::*,
		schemas::{steam_id64_to_account_id, steam_id_to_account_id, FancyServer},
	},
	gokz_rs::prelude::*,
	log::debug,
	serde::Deserialize,
};

#[derive(Debug, Deserialize)]
pub(crate) struct Params {
	pub(crate) name: Option<String>,
	pub(crate) owned_by: Option<String>,
	pub(crate) approved_by: Option<String>,
	pub(crate) limit: Option<u32>,
}

pub(crate) async fn get(
	Query(params): Query<Params>,
	State(GlobalState { pool }): State<GlobalState>,
) -> Response<Vec<FancyServer>> {
	let start = Utc::now().timestamp_nanos();
	debug!("[servers::get]");
	debug!("> `params`: {params:#?}");

	let mut filter = String::new();
	if let Some(name) = params.name {
		filter.push_str(&format!(r#"AND server.name LIKE "%{name}%""#));
	}

	if let Some(owned_by) = params.owned_by {
		let ident = PlayerIdentifier::try_from(owned_by)?;
		filter.push_str(&format!(
			"AND server.{} ",
			match ident {
				PlayerIdentifier::Name(name) => {
					format!(r#"owner_name LIKE "%{name}%" "#)
				}
				PlayerIdentifier::SteamID(steam_id) => {
					let account_id = steam_id_to_account_id(&steam_id.to_string())
						.ok_or(eyre!("Invalid SteamID"))?;
					format!(r#"owner_id = {account_id} "#)
				}
				PlayerIdentifier::SteamID64(steam_id64) => {
					let account_id = steam_id64_to_account_id(steam_id64)?;
					format!(r#"owner_id = {account_id} "#)
				}
			}
		));
	}

	if let Some(approved_by) = params.approved_by {
		let ident = PlayerIdentifier::try_from(approved_by)?;
		filter.push_str(&format!(
			"AND server.{} ",
			match ident {
				PlayerIdentifier::Name(name) => {
					format!(r#"approved_by_name LIKE "%{name}%" "#)
				}
				PlayerIdentifier::SteamID(steam_id) => {
					let account_id = steam_id_to_account_id(&steam_id.to_string())
						.ok_or(eyre!("Invalid SteamID"))?;
					format!(r#"approved_by_id = {account_id} "#)
				}
				PlayerIdentifier::SteamID64(steam_id64) => {
					let account_id = steam_id64_to_account_id(steam_id64)?;
					format!(r#"approved_by_id = {account_id} "#)
				}
			}
		));
	}

	let filter = format!(
		"\n{}\nLIMIT {}",
		filter.replacen("AND", "WHERE", 1),
		params.limit.unwrap_or(9999)
	);

	let servers = get_servers(QueryInput::Filter(filter), &pool).await?;

	Ok(Json(ResponseBody {
		result: servers,
		took: (Utc::now().timestamp_nanos() - start) as f64 / 1_000_000f64,
	}))
}
